...
class _GroupAndTagsState extends State<GroupAndTags> {
  ...
  Future<void> Groupadd() async {
    try {
      if (_form.currentState!.validate()) {
        ...
        final groupPost = await GroupApi.postGrroup(
        ...
        if (groupPost.success == 1) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text( "${groupPost.msg}Failed to add group"),
            ),
          );
          Navigator.pop(context);
        }
  ...
  Future<void>TagAdd()async{
    try{
      if(_form.currentState!.validate()){
        final tagpost = await TagApi.PostTag(
        ...
        if(tagpost.success == 1){
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('${tagpost.msg}Failed to Add Tag'))
          );
          Navigator.pop(context);
        }
      }
                        ...
                        onTap: (){
                          showDialog(context: context, builder:(context){
                            return Form(
                              key: _form,
                              child: AlertDialog(
                               title: Text('ADD GROUP',style: GoogleFonts.poppins(fontSize: 14,fontWeight: FontWeight.w500),textAlign:TextAlign.center),
                                ...
                                actions: [
                                  CommonButton(
                                    bordercircular: 5,
                                      ...
                                      child: Text('Save',style: GoogleFonts.poppins(color: Colors.white,fontSize: 12),))
                                ],
                              ),
                            );
                          });
                      ...
                      onTap: (){
                        showDialog(context: context, builder:(context){
                          return Form(
                            key: _form,
                            child: AlertDialog(
                              title: Text('ADD TAG',style: GoogleFonts.poppins(fontSize: 14,fontWeight: FontWeight.w500),textAlign:TextAlign.center),
                              content: CommonTextForm(
                                contentpadding: 5,
