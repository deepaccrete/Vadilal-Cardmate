...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Map<String, dynamic> dataCardToExportableMap(DataCard card) {
    return {
      ...
      'Person Designations':
          card.personDetails?.map((p) => p.position).join(', ') ?? '',
      'Person Phone ':
          card.personDetails?.map((p) => p.phoneNumber).join(',') ?? '',
      ...
      if (data.isNotEmpty) {
        ...
        if (io.Platform.isAndroid || io.Platform.isIOS) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Excel file saved Successfully')),
          );
        }
      } else {
                ...
                InkWell(
                  onTap: () {
                    List<Map<String, dynamic>> exportableData =
                        _cardapi
                            .map((card) => dataCardToExportableMap(card))
                            .toList();
                    exportDataToExcel(
                      context,
                      _cardapi.map((card) => card.toJson()).toList(),
                    );
                  },
                  child: Card(
                    elevation: 10,
